...
mod arch {
    ...
    /// SIMD base94 encode fast path (see `ssea::base94_encode_into`).
    ///
    ...
    pub(super) fn base94_encode_simd(
        dst_ptr: *mut u8,
        pos: &mut usize,
        src: &[u8],
        kf8: u8,
        ...
        // SAFETY: SSE2 is baseline on x86_64. Loads stay inside `src`; the
        // 32-byte scratch is a stack array; stores into `dst_ptr` stay
        // within the caller-reserved `2*src.len()+1` bytes (each block emits
        // at most 32 bytes; over-stored compact padding is overwritten by
        // later quarters or cut by the caller's final `set_len`).
        unsafe {
    ...
    /// SIMD base94 decode fast path (see `ssea::base94_decode_into`).
    ///
    ...
    #[allow(clippy::too_many_lines)]
    pub(super) fn base94_decode_simd(
        dst_ptr: *mut u8,
        pos: &mut usize,
        src: &[u8],
        kf8: u8,
        ...
        // SAFETY: SSE2 is baseline on x86_64. All loads are 16-byte block
        // loads inside `src`; all stores stay within the `src.len()` bytes
        // the caller reserved past `*pos` (each block emits < 16 bytes, and
        // over-stored padding bytes are overwritten by later blocks or cut
        // by the caller's final `set_len`).
        unsafe {
            let flip = _mm_set1_epi8(-0x80_i8);
            ...
            while block < n_blocks {
                ...
                // compares live in i16 lanes: narrow back to byte lanes
                // (packus_epi16 saturates 0xFFFF -> 0xFF, keeping the sign
                // bit movemask needs) before intersecting with the
                // byte-lane follower mask.
                let ovf_lo = _mm_cmpgt_epi16(ev_lo, c255);
                let ovf_hi = _mm_cmpgt_epi16(ev_hi, c255);
                let ovf_bytes = _mm_or_si128(
                    _mm_packus_epi16(ovf_lo, zero),
                    _mm_packus_epi16(zero, ovf_hi),
                );
                let bad_esc = _mm_movemask_epi8(_mm_and_si128(ovf_bytes, fmask)) as u32;
                if bad_lead != 0 || bad_esc != 0 {
...
/// SIMD base94 encode fast path (x86_64 only; other targets use the scalar
/// loop). Returns the consumed source prefix (multiple of 16).
pub(super) fn base94_encode_simd(dst_ptr: *mut u8, p: &mut usize, src: &[u8], kf8: u8) -> usize {
    #[cfg(target_arch = "x86_64")]
    {
        arch::base94_encode_simd(dst_ptr, p, src, kf8)
    }
...
/// SIMD base94 decode fast path (x86_64 only; other targets decode with the
/// scalar reference loop). See the x86_64 backend for semantics.
pub(super) fn base94_decode_simd(
    dst_ptr: *mut u8,
    p: &mut usize,
    src: &[u8],
    ...
    #[cfg(target_arch = "x86_64")]
    {
        arch::base94_decode_simd(dst_ptr, p, src, kf8)
    }
    #[cfg(not(target_arch = "x86_64"))]
    {
        let _ = (dst_ptr, p, src, kf8);
